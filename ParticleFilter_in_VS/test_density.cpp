...
int main(int argc,char *argv[]) {
  ...
  // Set Process Noise
  // ==============================
  cv::Mat ProcessCov        = (cv::Mat_<double>(1, 1) << 10); // random walk.
  ...
  cv::Mat ObsCov        = (cv::Mat_<double>(observation_dimension, 1) 
						   << 1, 0.01, 0.01); // three sensor model.
  ...
  for(int loop = 0; loop < NumOfIterate; loop++){
	...
	for (k = 0; k < T; k += 1.0){
	  ...
	  pfm.KernelDensityEstimation(predictionMeanshiftEst, densities, maps,process, Trans_likelihood);
	  ...
	  for (int i = 0; i < pfm.samples_; i++){
		particles_file << pfm.filtered_particles[i].state_.at<double>(0, 0) << " " 
					   ...
					   << exp(pfm.predict_particles[i].weight_) << endl;
	  }
...
// #endif // PARTICLE_IO
	  ...
	  ml_rmse.storeData(state.at<double>(0, 0), predictionML.at<double>(0,0));
	  ...
	  output << state.at<double>(0, 0) << " "       // [1] true state
			 ...
			 << predictionPF.at<double>(0,0) << " " // [3] predicted state by PF(MMSE)
			 << 0 << " "              // [4] predicted state by EPVGM
			 << 0 << " "              // [5] predicted state by PFMAP
			 << predictionML.at<double>(0,0) << " " // [6] predicted state by PF(ML)
			 << predict_x_ms << " "                 // [7] predicted state by PF(MS)
			 << measurement.at<double>(1, 0) << " " // [8] second sensor
			 << measurement.at<double>(2, 0) <<//  " " // [9] third sensor
		endl; 
	  output_diff << state.at<double>(0, 0) - 0 << " "
				  << state.at<double>(0, 0) - 0 << endl;
	  last_state = state;
