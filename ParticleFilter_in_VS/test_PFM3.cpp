...
int main(void) {
	...
	cv::Mat ObsCov = (cv::Mat_<double>(3, 1) << sqrt(10.0), sqrt(10.0), sqrt(10.0));
	...
	for (;;)
	{
		...
		setIdentity(KF.measurementNoiseCov, Scalar::all(10.0));
		...
		for (;;)
		{
			...
			// EP-VGM Estimation
			epvgm.Recursion(pfm, observed, input);
			Mat predictionEPVGM = epvgm.GetEstimation();
			double predictEPVGM_x1 = predictionEPVGM.at<double>(0, 0);
			double predictEPVGM_x2 = predictionEPVGM.at<double>(1, 0);
			double predictEPVGM_x3 = predictionEPVGM.at<double>(2, 0);
			...
			output 
				...
				<< predictPF_x1 << " "		// [7] PF(MMSE) x1
				<< predictPF_x2 << " "		// [8] PF(MMSE) x2
				<< predictPF_x3 << " "		// [9] PF(MMSE) x3
				<< predictEPVGM_x1 << " "	// [10] EPVGM x1
				<< predictEPVGM_x2 << " "	// [11] EPVGM x2
				<< predictEPVGM_x3 << endl;	// [12] EPVGM x3
	...
	system("wgnuplot -persist plot3.plt");
